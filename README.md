...
## Обновление v23.4.26: права на изменение статуса задачи в общем пуле

Версия исправляет ошибку:

```text
Факт в текущем спринте сохранён, но не удалось обновить общий пул:
Missing or insufficient permissions.
```

Причина: старый код при изменении статуса планирования отправлял в Firestore весь нормализованный документ задачи. Для части ранее созданных задач это затрагивало отсутствующие служебные поля Team_poker, и Firestore Rules отклоняли запрос.

В v23.4.26:

- браузер обновляет только поля планирования;
- Firestore Rules разрешают только этот ограниченный набор полей;
- если спринт уже сохранил возврат задачи, а пул не обновился, в карточке появится кнопка **«Повторить возврат в пул»**.

### Обязательно при публикации

1. Заменить `index.html`.
2. Открыть Firebase Console → Firestore Database → Rules.
3. Опубликовать `firestore.rules` из этого комплекта.
4. Обновить страницу приложения через `Cmd + Shift + R`.


## Обновление v23.4.27: корректные показатели завершённого спринта

Исправлена ситуация, когда после завершения спринта невыполненные задачи возвращались в общий пул, исключались из расчётного состава, и блок «Загрузка спринта по ролям» показывал 100% выполнения.

Теперь после завершения:

- план по ролям считается по всем задачам, участвовавшим в спринте;
- возвращённые в пул задачи остаются в знаменателе и считаются невыполненными;
- факт сохраняет принятые трудозатраты по всем участвовавшим задачам;
- общие счётчики и сохранённый результат спринта используют ту же логику;
- для активного спринта показатели по-прежнему строятся по текущему составу.

Изменения Firestore Rules не требуются.
